...
# =========================================================================
# sysopen O_CREAT checks parent directory perms (GH #329)
# =========================================================================
...
subtest 'sysopen on existing file checks file perms, not parent' => sub {
    my $parent = Test::MockFile->new_dir( '/perms/sdir3', { mode => 0111, uid => 1000, gid => 1000 } );
    my $child  = Test::MockFile->file( '/perms/sdir3/existing', 'data', { mode => 0644, uid => 1000, gid => 1000 } );

    with_user {
        ok( sysopen( my $fh, '/perms/sdir3/existing', O_RDONLY ), 'sysopen RDONLY on existing file checks file perms' );
        close $fh if $fh;
    } 1000, 1000;

    with_user {
        ok( !sysopen( my $fh, '/perms/sdir3/existing', O_WRONLY ), 'sysopen WRONLY fails without file write perm' );
        is( $! + 0, EACCES, 'errno is EACCES for write on read-only file' );
    } 2000, 2000;
};
